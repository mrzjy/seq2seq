  ...
  """
    ...
    """
        ...
        with tf.variable_scope("Transformer", initializer=initializer):
            ...
            # sequence is known.
            if targets is None:
                if self.params["get_all_beams"]:  # all to beam-major
                    # [batch, beam, length] to [beam, batch, length]
                    predicts = self.predict(encoder_outputs, attention_bias, get_all_beams=True)
                    ...
                    predicts = self.predict(encoder_outputs, attention_bias, get_all_beams=False)
                    length = Model_graph_utils.get_seq_length(predicts)
                return predicts, length
            else:
                logits = self.decode(targets, encoder_outputs, attention_bias)
                return logits, Model_graph_utils.get_seq_length(targets)
    ...
    def predict(self, encoder_outputs, encoder_decoder_attention_bias, get_all_beams=False):
        ...
        top_scores = scores[:, 0]
        return decoded_ids[:, :, 1:] if get_all_beams else top_decoded_ids
...
def sequence_beam_search(
        ...
        length_penalty_weight, max_decode_length, eos_id):
