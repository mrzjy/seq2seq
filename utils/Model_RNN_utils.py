...
class Embedding(tf.layers.Layer):
    ...
    def linear(self, x):
        ...
        # return tf.matmul(x, self.embedding, transpose_b=True)
...
def build_MultiRNNCell(input_size, hidden_size, num_hidden_layers, mode, dropout=0.2,
    ...
    if mode == tf.contrib.learn.ModeKeys.TRAIN and variational_dropout:
        # wrap = lambda cell: cell
        wrap = lambda cell: tf.nn.rnn_cell.DropoutWrapper(
    ...
    return tf.contrib.rnn.MultiRNNCell(
        [wrap(make_cell(hidden_size)) for _ in range(num_hidden_layers)], state_is_tuple=state_is_tuple)
